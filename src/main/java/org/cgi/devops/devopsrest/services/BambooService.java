...
public class BambooService {
	...
	public void bambooTrigger(BambooSpecs bambooSpecs){
       // BambooServer bambooServer = new BambooServer(bambooSpecs.getBambooServerUrl(),(UserPasswordCredentials) new UsernamePasswordCredentials(bambooSpecs.getBambooUsername(), bambooSpecs.getBambooPassword())); 
	...
	public PlanPermissions createPlanPermission(PlanIdentifier planIdentifier) {
	        Permissions permission = new Permissions()
	                ...
	                .loggedInUserPermissions(PermissionType.ADMIN)
	 ...
	 public Project project(BambooSpecs bambooSpecs) {
	        return new Project()
	                .name(bambooSpecs.getBambooProjectName())
	                .key(bambooSpecs.getBambooProjectKey());
	    }
...
//	    	BitbucketServerRepository repositories = new BitbucketServerRepository().server(server);
			return new Plan(project(bambooSpecs),bambooSpecs.getBambooPlanName(),bambooSpecs.getBambooPlanKey())
							.planRepositories(new BitbucketServerRepository()
	                	       .name(bambooSpecs.getBitbucketRepoName())
	                	       .server(serverB)
	                	       .projectKey(bambooSpecs.getBitbucketProjectKey())
	                	       .repositorySlug(bambooSpecs.getBitbucketRepoSlug())
	                	       .branch(bambooSpecs.getBitbucketRepoBranch()))
							.description("Plan created from (my desc)")
							.enabled(true)
							.stages(
	        					new Stage("my first stage").jobs(new Job(bambooSpecs.getBambooJobName(),bambooSpecs.getBambooJobKey())
	        							.tasks(new VcsCheckoutTask()
	        									.cleanCheckout(false)
	        									.checkoutItems(new CheckoutItem().defaultRepository()
	        											,new CheckoutItem()
	        									.repository(bambooSpecs.getBitbucketRepoName())
	        									.path("../../../../Devd/")),
	        									new MavenTask().goal("sonar:sonar package").hasTests(false)
	        											.version3().jdk("JDK 1.8").executableLabel("Maven 3")
	        											,new DockerBuildImageTask().imageName("cyan").dockerfileInWorkingDir()
	        											,new DockerRunContainerTask()
	        												.imageName("cyan")
	        												.additionalArguments("-d -p 8090:8121 --name=cyan_shanon")
	        												.containerEnvironmentVariables("ENV1=devendra ENV3=sankho ENV4=sankho ENV2=sankho mahi=mahesh WORDPRESS_DB_HOSTURL=gotyaa WORDPRESS_DB_=praneeth")
	        											,new ScriptTask().inlineBody("echo Hello Devendra!!")))
	        					)
